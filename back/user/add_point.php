...
// fetch promotion
$sql2 = "SELECT * FROM promotion order by id";
...
if (isset($_POST['submit'])) {
        ...
        if (!$error) {
        if (mysqli_query($con, "UPDATE users SET own_point = own_point+$add WHERE id = " . $id)){
          mysqli_query($con, "INSERT INTO point_history(user_id,add_point,promotion,note,admin) VALUES('".$id."','".$add."','".$promotion."','".$_POST['note']."','".$_SESSION['admin_usrname']."')");
          //$successmsg = "เพิ่มแต้มเรียบร้อยแล้ว <a href='index.php'>กลับ</a>";
  ...
  <div class="row">
    <div class="col-lg-4" style="margin-top:20px; margin-left:20px;">
      <ul class="list-group">
                    ...
                    <li class="list-group-item"><strong>แต้มที่มี</strong> : <?php if(isset($row['own_point'])) { echo $row['own_point']; } else { echo $_POST['own_point']+$add; } ?></li>
                </ul>
    </div>
  </div>
	<div class="row">
		<div class="col-lg-4 col-lg-offset-4 card card-body bg-light" style="margin-top:20px; margin-left:20px;">
            ...
            <input type="hidden" name="own_point" value="<?php if(isset($row['own_point'])) { echo $row['own_point']; } else { echo $_POST['own_point']+$add; } ?>" />
          </div>
          <div class="input-group mb-3">
            <div class="input-group-prepend">
            <span class="input-group-text">เพิ่ม</span>
            </div>
            <input type="number" name="add" class="form-control add" placeholder="" min="1" value="1">
          ...
          <div class="form-group">
            ...
            <select name="promotion" class="form-control input-sm">
                  ...
                  <option value="<?php echo $row2['detail']; ?>"><?php echo $row2['detail']; ?></option>
                <?php } ?>
            </select>
          </div>
